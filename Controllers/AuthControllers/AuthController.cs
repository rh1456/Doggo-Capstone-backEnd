...
using Doggo_Capstone_backEnd.Services;
...
namespace Doggo_Capstone_backEnd.Controllers
{
  ...
  public class AuthController : ControllerBase
  {
    ...
    [HttpPost("signup")]
    public async Task<ActionResult> SignUpUser(UserDataViewModel userData)
    {
    ...
    public async Task<ActionResult> LoginUser(LoginViewModel loginData)
    {
      ...
      var validEntry = new PasswordHasher<User>().VerifyHashedPassword(user, user.HashedPassword, loginData.Password);

      if (validEntry == PasswordVerificationResult.Success)
      {
        var rv = new AuthService(this.configuration).CreateToken(user);
        return Ok(rv);
      }
